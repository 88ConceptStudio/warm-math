...
from vector import Vector, dodawanie, mnozenie, dzielenie, odejmowanie
...
def index():
    ...
    if request.method == 'POST':
        try:
            ...
            # Wykonaj operację
            if op == 'add':
                result = dodawanie(v1, v2)
            elif op == 'mul':
                result = mnozenie(v1, v2)
            elif op == 'div':
                result = dzielenie(v1, v2)
            elif op == 'sub':
                result = odejmowanie(v1, v2)
            ...
            result_str = "Błąd: Wprowadź poprawne liczby całkowite."
    
    return render_template('index.html', result=result_str if 'result_str' in locals() else None, plot_url=plot_url)
